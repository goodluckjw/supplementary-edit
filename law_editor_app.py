...
def fetch_law_text(law_name):
    return f"{law_name} 제1조(목적) ① 이 법은 {law_name}에 관한 사항을 정한다. ② 지방법원은 이에 따른다."

def process_excel_laws(file, original_term, replacement_term):
    df = pd.read_excel(file, header=None)
    law_names = df.iloc[6::2, 2].dropna().tolist()

    output_lines = []
    for idx, law_name in enumerate(sorted(set(law_names)), 1):
        text = fetch_law_text(law_name)
...
### 🧾 사용 방법:
1. **국회 입법정보시스템에서 받은 엑셀 파일을 업로드**합니다.  
...
if uploaded_file and original_term and replacement_term:
    result_text = process_excel_laws(uploaded_file, original_term, replacement_term)
    ...
    st.info("👆 엑셀 파일과 단어를 모두 입력해주세요. 예시가 기본으로 들어가 있습니다.")
